...
class CSNetHRLite(object):
    ...
    def se_block(self, x: tf.Tensor, block_id: str):
        h_axis, w_axis = [1, 2]
        filters = x.shape[-1]
        reduction = int(filters * 0.25)
    ...
    def conv_block(self, x: tf.Tensor, in_filters: int,  out_filters: int,
        ...
        if stride == 2:
            x = tf.keras.layers.ZeroPadding2D(padding=self.correct_pad(x, kernel_size),
                                              name='std_conv_pad_{0}'.format(block_id))(x)
        ...
        x = tf.keras.layers.Conv2D(filters=out_filters,
                                   ...
                                   use_bias=False, name='std_conv_{0}'.format(block_id))(x)
        x = self.batch_norm(epsilon=self.EPSILON,
                            momentum=self.MOMENTUM,
                            name='std_bn_{0}'.format(block_id))(x)
        x = tf.keras.layers.Activation(self.activation, name='std_activation_{0}'.format(block_id))(x)

        if use_se:
            x = self.se_block(x=x, block_id=block_id)
        ...
        if in_filters == out_filters and stride == 1:
            ...
            x = tf.keras.layers.Add(name='std_add_{0}'.format(block_id))([shortcut_tensor, x])
        return x

    def sep_conv_block(self, x: tf.Tensor, in_filters: int, out_filters: int, expand_ratio: int,
                   kernel_size: int, stride: int, block_id: int, dilation_rate: int = 1, use_se: bool = False):
        shortcut_tensor = x
    ...
    def aspp(self, x: tf.Tensor, in_filters: int, out_filters: int, dropout_rate: float) -> tf.Tensor:
        ...
        b1 = self.sep_conv_block(x=x, in_filters=in_filters, out_filters=out_filters, expand_ratio=1, kernel_size=3, stride=1, block_id='aspp_b1')
        b2 = self.sep_conv_block(x=x, in_filters=in_filters, out_filters=out_filters, expand_ratio=1, kernel_size=5, stride=1, block_id='aspp_b2')
        b3 = self.sep_conv_block(x=x, in_filters=in_filters, out_filters=out_filters, expand_ratio=1, kernel_size=7, stride=1, block_id='aspp_b3')
    ...
    def build_model(self, hp=None) -> tf.keras.models.Model:
        ...
        # Stage 2: 64x64 os4
        stage2 = self.conv_block(x=stage1, in_filters=stage1_units, out_filters=stage2_units, kernel_size=stage2_kernel, stride=2, block_id='stage2_in')
        stage2 = self.conv_block(x=stage2, in_filters=stage2_units, out_filters=stage2_units, kernel_size=stage2_kernel, stride=1, block_id='stage2_{0}'.format(1))
        
        # Stage 3: 32x32 os8
        stage3 = self.sep_conv_block(x=stage2, in_filters=stage2_units, out_filters=stage3_units, expand_ratio=stage3_expand, kernel_size=stage3_kernel, stride=2, block_id='stage3_in')
        for i in range(stage3_layers):
            stage3 = self.sep_conv_block(x=stage3, in_filters=stage3_units, out_filters=stage3_units, expand_ratio=stage3_expand, kernel_size=stage3_kernel, stride=1, block_id='stage3_{0}'.format(i+1))
        
        # Stage 4: 16x16 os16
        stage4 = self.sep_conv_block(x=stage3, in_filters=stage3_units, out_filters=stage4_units, expand_ratio=stage4_expand, kernel_size=stage4_kernel, stride=2, block_id='stage4_in', use_se=True)
        for i in range(stage4_layers):
            stage4 = self.sep_conv_block(x=stage4, in_filters=stage4_units, out_filters=stage4_units, expand_ratio=stage4_expand, kernel_size=stage4_kernel, stride=1, block_id='stage4_{0}'.format(i+1), use_se=True)

        # Stage 5: 16x16 os16
        stage5 = self.sep_conv_block(x=stage4, in_filters=stage4_units, out_filters=stage5_units, expand_ratio=stage5_expand, kernel_size=stage5_kernel, stride=1, block_id='stage5_in', use_se=True)
        ...
        x = self.sep_conv_block(x=x, in_filters=stage3_units, out_filters=stage3_units, expand_ratio=1, kernel_size=3, stride=1, block_id='decode_os8_output')

        # os8 decode -> 64x64 (os4)
        x = self.deconv_block(x=x, out_filters=stage2_units, block_id='decode_os4_deconv')
        x = self.fusion_block(x=x, skip=stage2, in_filters=stage2_units, kernel_size=3, block_id='decode_os4_fusion')
        x = self.sep_conv_block(x=x, in_filters=stage2_units, out_filters=stage2_units, expand_ratio=1, kernel_size=3, stride=1, block_id='decode_os4_output')
