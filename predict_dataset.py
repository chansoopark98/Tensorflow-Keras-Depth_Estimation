...
parser.add_argument("--weight_name",         type=str,    help="Saved model weights directory",
                    default='_Bs-32_Ep-100_Lr-0.0002_ImSize-480_Opt-adamW_multi-gpu_0315_230315_EfficientV2B0_customLoss_480x640_adam_lossFactor_test2_best_loss.h5')
...
if __name__ == '__main__':
    ...
    # # Set colormap
    sm = plt.cm.ScalarMappable(cmap='plasma', norm=plt.Normalize(vmin=0, vmax=10.0))
    # plt.colorbar(sm)
    ...
    for img, depth in test_data.take(100):
        ...
        depth = (1. / depth) * 10.
        pred = (1. / pred) * 10.
        ssim_value = tf.image.ssim(depth, pred, 10., filter_size=7, k1=0.01 ** 2, k2=0.03 ** 2)
        ...
        ax0.imshow(pred[0], cmap='plasma', vmin=0.0, vmax=10.)
        ax0.set_title('pred_depth')
        ax0.axis("off")

        ax0 = fig.add_subplot(rows, cols, 3)
        ax0.imshow(depth[0], cmap='plasma', vmin=0.0, vmax=10.)
        ax0.set_title('gt')
        ax0.axis("off")
